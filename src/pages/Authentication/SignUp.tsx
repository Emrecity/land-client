...
const SignUp: React.FC = () => {
  ...
  const mutation = useMutation({
    mutationFn: async (data: FormValues) => {
      ...
      const response = await axios.post('/api/v2/auth/signup', formData);
          ...
          <div className="w-full border-stroke dark:border-strokedark xl:w-1/2 xl:border-l-2">
            <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
              <h1 className="font-semibold text-2xl text-gray-600 mb-4 capitalize">
                sign up to OpenCast<span className="text-yellow-400">GH</span>
              </h1>
              <form onSubmit={handleSubmit(onSubmit)}>
                <div className="mb-4">
                  ...
                  <div className="relative">
                    ...
                    />
                    {errors.fullName?.message && <p className="text-red-500">{errors.fullName.message.toString()}</p>}
                ...
                <div className="mb-4">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full rounded-lg border border-stroke bg-transparent py-2 pl-6 pr-10 text-black outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                    />
                    {errors.email?.message ? <p className="text-red-500">{errors.email.message.toString()}</p> : null}
                    <span className="absolute right-4 top-2">
                      <svg
                        className="fill-current"
                        width="22"
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full rounded-lg border border-stroke bg-transparent py-2 pl-6 pr-10 text-black outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                    />
                    {errors.phone?.message && <p className="text-red-500">{errors.phone.message.toString()}</p>}
                    <span className="absolute right-4 top-2">
                      <BsPhone />
                    </span>
                  </div>
                ...
                <div className="mb-6">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full rounded-lg border border-stroke bg-transparent py-2 pl-6 pr-10 text-black outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                    />
                    {errors.role?.message && <p className="text-red-500">{errors.role.message.toString()}</p>}
                    <span className="absolute right-4 top-2">
                    </span>
                  </div>
                </div>
                ...
                <div className="mb-5">
                  ...
                  >{mutation.isPending ? <LoaderIcon /> : 'Sign Up'}</button>
                </div>
