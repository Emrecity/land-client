...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  return (
    <aside
      ref={sidebar}
      className={`fixed lg:static left-0 top-20 z-50 flex justify-between h-screen overflow-y-auto overflow-x-hidden ${
        isExpanded ? 'w-60' : 'w-20'
      } flex-col overflow-y-auto bg-black duration-300 ease-linear dark:bg-boxdark ${
        sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
      }`}>
      <button
        onClick={() => setIsExpanded(!isExpanded)}
        className="absolute right-3 top-3 transform translate-x-1/2 bg-gray-800 p-2 rounded-full  hidden lg:block"
      >
        {isExpanded ? (
          <X size={10} className="text-gray-300" />
        ) : (
          <Menu size={10} className="text-gray-300" />
        )}
      </button>
